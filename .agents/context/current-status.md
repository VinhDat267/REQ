...
- `Group1_Tutorial01_BRD_Ver3.docx` was text-synced on `2026-04-17` against `UC_Specifications_Part1_EN.md` and `UC_Specifications_Part2_EN.md`:
  ...
  - after a source-control discard removed the same-day BRD edits, `Group1_Tutorial01_BRD_Ver3.docx` was re-fixed on `2026-05-04`: Appendix B now contains a real `Business Rules Catalog` followed by `Business Case Summary`, client / stakeholder identity is aligned to `Wonton Noodle Shop`, business-case wording is reconciled with academic prototype constraints, scope-risk phrases such as full inventory management / multi-wallet integration / hardware procurement were removed, stale TOC text and UC blank cells were cleaned, and verification via `python-docx` shows `42` tables, a `12`-rule business-rules table, full `UC-01..UC-26` coverage, no extra UC numbers, no empty UC spec cells, and no tracked-change or comment markup.
  - the BRD `TABLE OF CONTENTS` was also rebuilt on `2026-05-04` as a Word automatic TOC with hyperlinks and page numbers; top-level document sections use proper `Heading 1..3` styles, individual Appendix A `Use Case 01..26` headings were moved to `Heading 4` so the TOC remains concise, stale TOC entries were removed, and the TOC was manually refreshed after later formatting changes while open-time field refresh was disabled to avoid the Word update-field popup.
  - BRD formatting was standardized from Word page 3 onward on `2026-05-04`: body/table text is `Times New Roman` size `13`, while headings are larger (`Heading 1` `16`, `Heading 2` `15`, `Heading 3` `14`, `Heading 4` `13.5`) and the TOC title is `16` bold. Verification with Word automation showed `129` pages, updated TOC pagination, no temporary TOC marker, and no `updateFields` open-time prompt setting.
  ...
  - root `SRSReport_Class01_Group01.docx` was review-fixed on `2026-05-04` after the BRD was closed: all `26` UC specification tables now match the closed BRD titles, primary actors, and priorities; section `2.4.1 FUNCTIONAL REQUIREMENTS` now uses the same canonical UC names in `R.1..R.26`; section `2.4.2 NON-FUNCTIONAL REQUIREMENTS` is a traceable `NFR-01..NFR-09` table with priority, rationale, and related use cases; numbered SRS headings use Word heading styles; and course/repo-style phrases such as `final project`, `repository`, and `academic review` were removed from the checked body text.
  - SRS heading formatting was normalized on `2026-05-04` after review feedback that the main section headings were visually too large: `Heading 1` was reduced from `36` pt to `16` pt, `Heading 2` was set to `14` pt, and `Heading 3` / `Heading 4` were set to `13` pt, all in `Times New Roman` bold.
  - SRS sections `1.3 Definitions, Acronyms, and Abbreviations`, `2.2 SOFTWARE REQUIREMENT`, and `2.3 HARDWARE REQUIREMENT` were converted from bullet lists into formal two-column tables; verification showed one table per section, `44` definition rows plus header, `7` software rows plus header, `5` hardware rows plus header, no remaining bullet symbols, and no heading-styled paragraphs inside those tables.
...
- Current Final SRS DOCX artifact:
  - root `SRSReport_Class01_Group01.docx` has been review-aligned so section `2.8 USE CASE MODEL DESCRIPTION` now contains the full curated `UC-01..UC-26` specification lane in the existing 10-row table format
  ...
  - section `2.4.2 NON-FUNCTIONAL REQUIREMENTS` now contains `NFR-01..NFR-09` with priority, rationale, and use-case traceability
  ...
  - document heading sizes have been restrained for formal SRS layout: `Heading 1` `16`, `Heading 2` `14`, `Heading 3` `13`, `Heading 4` `13`, all `Times New Roman` bold
  - sections `1.3`, `2.2`, and `2.3` now use formal two-column tables instead of bullet lists
  - verification was completed at text/table level with `python-docx`: `26` UC tables, ordered `UC-01..UC-26`, each `10x2`, with no missing local final-lane UC; SRS UC title / primary actor / priority fields have `0` diff against the closed BRD
